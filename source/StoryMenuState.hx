	...
	function selectWeek()
	{
		if (weekUnlocked[curWeek])
		{
			...
			new FlxTimer().start(1, function(tmr:FlxTimer)
			{
				if (curWeek == 0)
            	{
	         	FlxG.switchState(new VideoState('assets/videos/scene1/vid.webm', new PlayState()));
        		LoadingState.loadAndSwitchState(new PlayState(), true);
	            }
				else if (curWeek == 1)
             	{
	         	FlxG.switchState(new VideoState('assets/videos/scene5/final-cutscene-for-real.webm', new PlayState()));
        		LoadingState.loadAndSwitchState(new PlayState(), true);
				}
				else
				{
